...
import { RECOMMEND, coachOf, givenNameOf, carePlan, mvpSteps,
         TEST_FEE, APPLIED_LINE } from '../data';
import { C } from '../theme';
 ...
 */
export default function Brief({ pKey, onBack, onTestPaid }) {
  ...
  return (
    <Box sx={{ height: '100%', overflowY: 'auto', bgcolor: '#FAF6ED' }}>
      <Box sx={{
        ...
        {/* ── THE ASK (docs/MVP_LONG_FLOW.md §3.1–3.2) ──
            The programme total is context; the only price asked for is the
            blood test. One number per moment, and the money mechanic in its
            one standard phrase. */}
        <Typography sx={{
          fontSize: 11.5, color: C.ink2, mt: 2.5, textAlign: 'center',
        }}>{s.prog} · SAR {total.toLocaleString()} · 12 weeks of care</Typography>

        <Box sx={{
          mt: 1.25, px: 2.25, py: 2, borderRadius: '18px', bgcolor: '#fff',
          boxShadow: '0 10px 30px -24px rgba(27,57,91,.55)',
        }}>
          <Typography sx={{
            fontFamily: '"Fraunces", serif', fontSize: 19, fontWeight: 600,
            color: C.deep, lineHeight: 1.2,
          }}>Start with your blood test. SAR {TEST_FEE}.</Typography>
          <Typography sx={{ fontSize: 12, fontWeight: 600, color: C.yellowDeep, mt: 0.6 }}>
            {APPLIED_LINE}
          </Typography>
          <Typography sx={{ fontSize: 12.5, lineHeight: 1.55, color: C.ink2, mt: 1 }}>
            {first} reviews your results with you before any treatment is
            confirmed.
          </Typography>
        </Box>

        {/* the four steps, patient verbs */}
        ...
        <Button fullWidth variant="contained" color="secondary" onClick={() => setPay(true)}
          ...
          sx={{ mt: 2.5, py: 1.4, fontSize: 15 }}>
          Book my blood test
        </Button>
        <Typography sx={{ fontSize: 11, color: C.ink2, mt: 1, textAlign: 'center' }}>
          SAR {TEST_FEE}. {APPLIED_LINE}
        </Typography>
      </Box>

      <PaySheet open={pay}
        item="At-home blood test with nurse"
        fee={String(TEST_FEE)}
        note={`${APPLIED_LINE} A nurse comes to you.`}
        onClose={() => setPay(false)} onDone={onTestPaid} />
    </Box>
  );
}
