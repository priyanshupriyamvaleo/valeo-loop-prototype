...
import { COACHES, GLP_QUIZ, quizRoute, reasonLine, bmiOf, USER } from '../data';
 ...
 * the close is "you're a match" or "a doctor first". Nothing stops here:
 * only a clinician may say no.
 *
 * ── THE ENGINE ──
...
export default function Coach({ onBack, onDone }) {
  ...
  const [ending, setEnding] = useState(null); /* 'plan' | 'doctor' */
  ...
  const answer = (label, value) => {
    ...
    /* No answer ends the conversation any more. A flag routes to a clinician
       at the close, so the patient always finishes the file — a half-answered
       intake is no use to the doctor who has to read it. */
    const r = quizRoute(next);

    /* the BMI moment: computed, announced, never asked */
    if (step.k === 'weight' || step.k === 'comorbid') {
      const b = bmiOf(next);
      const ok30 = b >= 30;
      const low = b < 27;
      next.bmiMoment = true;
      out.push({ say: true, paras: low
        ? [`Your BMI is ${b}.`,
           'That sits below where GLP-1 is normally prescribed, so a doctor will talk it through with you before anything else happens.',
           'It is a ten-minute call, included, and nothing to pay before it.']
        : [`Your BMI is ${b}.`,
    ...
    if (r.out === 'doctor') {
      out.push({ say: true, paras: [
        ...
        [{ b: `${reasonLine(r.reasons)} means a doctor should look before we go further.` }],
        'Ten minutes on a video call, included, and nothing to pay before it. Pick a time and I’ll set it up.',
      ] });
      setEnding('doctor');
    } else {
      out.push({ say: true, paras: [
