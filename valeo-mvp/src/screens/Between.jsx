...
import { GOALS, LIVE, PROTOCOLS, USER, coachOf } from '../data';
import { C } from '../theme';
...
export default function Between({ onStart, onBack }) {
  /* PEOPLE, not plans — Dr. Mahmoud authors two of the four. */
          ...
          <Rise on={at('note')}>
            <Box sx={{
              ...
              <Typography sx={{ fontSize: 14.5, lineHeight: 1.6, color: C.ink, mt: 1.75 }}>
                We’ll stay with you throughout your journey.
              </Typography>
              ...
              <Typography sx={{ fontSize: 14, lineHeight: 1.8, color: C.ink2, mt: 1.75 }}>
                ...
                or simply want to celebrate a milestone…
              </Typography>
...
function Fan({ team }) {
  ...
  const live = team[n];
  const runs = LIVE.filter((pk) => coachOf(pk) === live).map((pk) => PROTOCOLS[pk].t);
