...
export const STATES = [
  ...
  { id: 'K2_PLAN', lane: 'known', t: 'Monthly plan',
    enter: 'Safety screen clean. The monthly plan is resolved from what the patient asked for.',
    exit: 'Patient pays the monthly plan.' },
  ...
  { id: 'D3_RECOMMENDED', lane: 'resolve', t: 'Care brief',
    enter: 'Assessment written. The programme is shown with its full price as context.',
    exit: 'Patient books the blood test. SAR 499, applied toward the programme.' },
  { id: 'D5_LABS', lane: 'resolve', t: 'Labs pending',
    enter: 'Blood test paid. Nurse slot to pick; results review booked alongside.',
    exit: 'Sample collected at home and results uploaded by the lab.' },
  { id: 'D6_REVIEW', lane: 'resolve', t: 'Results review',
    enter: 'Results in, doctor first. The review visit is the conviction peak.',
    exit: 'Treatment confirmed, prescription signed, programme activated: balance of total minus 499.' },
...
function runState(r, pKey, ui) {
  switch (r.status) {
    case 'consulted': return 'D3_RECOMMENDED';
    case 'programme':
...
export const TRANSITIONS = [
  ...
  { event: 'TEST_PAID', actor: 'patient', from: 'D3_RECOMMENDED', to: 'D5_LABS', sim: true,
    writes: 'Order PAID (SAR 499), applied toward the programme',
    reason: 'No care brief is open',
    guard: (st, ui, ep) => ui.flow === 'brief'
      || (!!ep && ep.state === 'D3_RECOMMENDED'),
    fire: (ctx, ep) => ctx.payTest(ep && ep.pKey) },

  { event: 'PAYMENT_COMPLETED', actor: 'patient', from: 'K2_PLAN', to: 'K4_REVIEW', sim: true,
    writes: 'Order PAID (monthly plan)',
  ...
  { event: 'PROGRAMME_ACTIVATED', actor: 'patient', from: 'D6_REVIEW', to: 'M1_FULFILMENT', sim: true,
    writes: 'Order PAID (balance: programme total minus 499); fulfilment begins',
    reason: 'Treatment is not confirmed yet',
...
export const QUEUES = [
  ...
  { id: 'patient', t: 'Waiting patient',
    has: (ep) => ['D3_RECOMMENDED', 'K2_PLAN', 'K4A_CALL', 'M1_FULFILMENT'].includes(ep.state)
