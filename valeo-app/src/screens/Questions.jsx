...
const FULL = [
  ...
  { type: 'multi',  q: 'Let’s talk lifestyle', groups: ['work', 'smoke', 'drink'] },
  { type: 'multi',  q: 'And how you move', groups: ['move', 'train'] },
  { type: 'multi',  q: 'Food, sleep and stress', groups: ['food', 'sleep', 'stress'] },
  { type: 'goal',   k: 'goal',   q: 'What are you trying to change?' },
  { type: 'done',   h: 'Your twin has what it needs.' },
];

export default function Questions({ reveal, onFinish, onBack }) {
  const STEPS = reveal
    ...
    return (
      <Box sx={{
        ...
        <Button variant="contained" color="secondary" onClick={next}
          ...
          {step.type === 'done' ? 'Show me my twin matches' : 'Continue'}
        </Button>
      </Box>
    );
  }
